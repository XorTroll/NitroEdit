...
namespace ui::menu {
    ...
    namespace {
        ...
        ntr::gfx::abgr1555::Color *g_MenuArrowGfx = nullptr;
        ...
        void StartDialogImpl(const std::string &msg) {
            ...
            ASSERT_TRUE(g_TextFont.RenderText(msg, 0, 0, NormalLineHeight, g_DialogBox.width, ShowDialogOnSub, ntr::gfx::abgr8888::BlackColor, true, g_DialogBoxText));
    ...
    void Initialize() {
        ...
        ASSERT_TRUE(g_MainBackground.LoadFrom("ui/main_bg.png", CreateSelfNitroFsFileHandle()));
        g_MainBackground.Refresh();
        
        g_SubBackground.Create(true);
        ASSERT_TRUE(g_SubBackground.LoadFrom("ui/sub_bg.png", CreateSelfNitroFsFileHandle()));
        g_SubBackground.Refresh();

        ASSERT_TRUE(g_TextFont.LoadFrom("ui/Nintendo-DS-BIOS.ttf", CreateSelfNitroFsFileHandle()));

        ASSERT_TRUE(LoadPNGGraphics("ui/menu_arrow.png", CreateSelfNitroFsFileHandle(), g_MenuArrowWidth, g_MenuArrowHeight, g_MenuArrowGfx));
        
        ASSERT_TRUE(g_DialogBox.CreateFrom(ShowDialogOnSub, 0, 0, "ui/dialog_box.png", CreateSelfNitroFsFileHandle()));
        g_DialogBox.CenterInScreen();

        ASSERT_TRUE(g_ScrollMenuItemSelectBox.CreateFrom(true, ScrollMenuItemBaseX, ScrollMenuItemBaseY, "ui/select_box.png", CreateSelfNitroFsFileHandle()));
    ...
    void PushMenu(const std::string &name) {
        ...
        if(!g_Menus.empty()) {
            ...
            ASSERT_TRUE(g_TextFont.RenderText(name, menu_arrow.x + menu_arrow.width, 0, NormalLineHeight, ScreenWidth, false, ntr::gfx::abgr8888::BlackColor, false, menu_spr));
            menu_spr.CenterYInSprite(menu_arrow);

            g_Menus.push_back(std::move(menu_arrow));
        }
        else {
            ASSERT_TRUE(g_TextFont.RenderText(name, 8, 8, NormalLineHeight, ScreenWidth, false, ntr::gfx::abgr8888::BlackColor, false, menu_spr));
        }
    ...
    void AddButtonText(const std::string &key, const std::string &text) {
        ...
        ASSERT_TRUE(g_TextFont.RenderText(key_text, 0, 0, NormalLineHeight, ScreenWidth, false, ntr::gfx::abgr8888::BlackColor, false, button_text_spr));
