...
namespace ui {
    ...
    bool Sprite::Load() {
        ...
        for(u32 i = 0; i < this->sprite_w * this->sprite_h; i++) {
            ...
            this->sprites[i].gfx_ptr = reinterpret_cast<ntr::gfx::abgr1555::Color*>(oamAllocateGfx(oam, BaseSpriteSize, SpriteColorFormat_Bmp));
    ...
    void Sprite::Unload() {
        if(this->sprites != nullptr) {
            auto oam = this->is_sub ? &oamSub : &oamMain;
            for(u32 i = 0; i < this->sprite_w * this->sprite_h; i++) {
                dmaFillHalfWords(0, this->sprites[i].gfx_ptr, BaseSpriteSizeValue * BaseSpriteSizeValue * sizeof(ntr::gfx::abgr1555::Color));
