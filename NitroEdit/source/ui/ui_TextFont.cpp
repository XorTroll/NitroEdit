...
namespace ui {
    ...
    bool TextFont::LoadFrom(const std::string &path, std::shared_ptr<ntr::fs::FileHandle> file_handle) {
        ntr::fs::BinaryFile ttf_bf;
        if(!ttf_bf.Open(file_handle, path, ntr::fs::OpenMode::Read)) {
            return false;
        }
    ...
    bool TextFont::RenderTextBase(const std::string &text, const u32 x, const u32 y, const u32 line_height, const u32 max_line_width, const bool is_sub, const ntr::gfx::abgr1555::Color clr, const bool centered, Sprite &out_text) {
