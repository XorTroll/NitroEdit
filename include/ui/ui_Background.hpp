...
namespace ui {
    ...
    struct Background {
        ...
        Background() : id(0), is_sub(false), width(0), height(0), gfx_ptr(), owns_gfx_ptr(false) {}
        ...
        ntr::Result LoadFrom(const std::string &path, std::shared_ptr<ntr::fs::FileHandle> file_handle) {
            this->Unload();
            NTR_R_TRY(LoadPNGGraphics(path, file_handle, this->width, this->height, this->gfx_ptr));

            this->owns_gfx_ptr = true;
            NTR_R_SUCCEED();
        }

        ntr::Result LoadFrom(ntr::gfx::abgr1555::Color *gfx_buf, const u32 width, const u32 height, const bool owns_gfx_buf = false) {
            this->Unload();
