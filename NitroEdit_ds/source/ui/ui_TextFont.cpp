...
namespace ui {
    ...
    ntr::Result TextFont::LoadFrom(const std::string &path, std::shared_ptr<ntr::fs::FileHandle> file_handle) {
        ntr::fs::BinaryFile ttf_bf;
        NTR_R_TRY(ttf_bf.Open(file_handle, path, ntr::fs::OpenMode::Read));
        size_t ttf_size;
        NTR_R_TRY(ttf_bf.GetSize(ttf_size));
        this->ttf_buf = ntr::util::NewArray<u8>(ttf_size);

        NTR_R_TRY(ttf_bf.ReadDataExact(this->ttf_buf, ttf_size));
        ttf_bf.Close();

        if(!stbtt_InitFont(&this->font, this->ttf_buf, 0)) {
            NTR_R_FAIL(ResultUiUnableToLoadFont);
        }
        NTR_R_SUCCEED();
    }

    ntr::Result TextFont::RenderTextBase(const std::string &text, const u32 x, const u32 y, const u32 line_height, const u32 max_line_width, const bool is_sub, const ntr::gfx::abgr1555::Color clr, const bool centered, Sprite &out_text) {
        ...
        auto gfx_buf = ntr::util::NewArray<ntr::gfx::abgr1555::Color>(max_width * max_height);
