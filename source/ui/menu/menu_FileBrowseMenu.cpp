...
namespace ui::menu {
    ...
    namespace {
        ...
        gfx::abgr1555::Color *g_DirectoryIconGfx = nullptr;
        gfx::abgr1555::Color *g_FileIconGfx = nullptr;
        gfx::abgr1555::Color *g_ROMIconGfx = nullptr;
        gfx::abgr1555::Color *g_MusicIconGfx = nullptr;
        gfx::abgr1555::Color *g_TexIconGfx = nullptr;
        gfx::abgr1555::Color *g_CurrentROMIconGfx = nullptr;

        ui::Sprite g_CurrentROMIcon = {};
        ui::Sprite g_CurrentROMText = {};

        struct NitroFsBrowseEntry {
            ntr::nfs::NitroFsFileFormat *file_ref_ptr;
            ntr::nfs::NitroDirectory *dir_ref_ptr;
            bool is_loaded_narc;
            std::string path;
        };

        std::stack<NitroFsBrowseEntry> g_NitroFsEntryStack;
        std::stack<ntr::NARC> g_LoadedNARCs;

        std::string g_CurrentDirectory;

        inline std::shared_ptr<fs::FileHandle> CreateCurrentNitroFsEntryFileHandle() {
            if(g_NitroFsEntryStack.top().is_loaded_narc) {
                auto &narc = *reinterpret_cast<ntr::NARC*>(g_NitroFsEntryStack.top().file_ref_ptr);
                return std::move(std::make_shared<ntr::NARCFileHandle>(narc));
            }
            else {
                auto &rom = *reinterpret_cast<ntr::ROM*>(g_NitroFsEntryStack.top().file_ref_ptr);
                return std::move(std::make_shared<ntr::ROMFileHandle>(rom));
            }
        }

        inline std::shared_ptr<fs::FileHandle> CreateCurrentFileHandleImpl(const bool is_fat) {
            if(is_fat) {
                return std::move(std::make_shared<fs::FatFileHandle>());
            }
            else {
        ...
        void OnFatDirectorySelect(const std::string &dir_path) {
            LoadFatFileBrowseMenu(fs::GetAbsolutePath(dir_path));
        }

        void OnNitroFsDirectorySelect(ntr::nfs::NitroFsFileFormat *file, ntr::nfs::NitroDirectory *dir) {
        ...
        void OnNARCSelect(const bool is_fat, const std::string &narc_path) {
            ...
            if(res == DialogResult::Yes) {
                ...
                if(st_narc.ReadFrom(narc_path, CreateCurrentFileHandleImpl(is_fat)) || st_narc.ReadFrom(narc_path, CreateCurrentFileHandleImpl(is_fat), fs::FileCompression::LZ77)) {
        ...
        bool ReadROMHeaderBanner(const bool is_fat, const std::string &path, ntr::ROM &out_rom) {
            fs::BinaryFile bf;
            if(!bf.Open(CreateCurrentFileHandleImpl(is_fat), path, fs::OpenMode::Read)) {
                return false;
            }
        ...
        void OnItemFocus(const bool is_fat, const std::string &path, const bool is_rom) {
            ...
            if(is_rom) {
                ...
                if(ReadROMHeaderBanner(is_fat, path, rom)) {
                    gfx::abgr8888::Color *icon_gfx;
                    if(gfx::ConvertBannerIconToRgba(rom.banner.icon_chr, rom.banner.icon_plt, icon_gfx)) {
                        g_CurrentROMIconGfx = util::NewArray<gfx::abgr1555::Color>(gfx::IconWidth * gfx::IconHeight);
                        for(u32 i = 0; i < gfx::IconWidth * gfx::IconHeight; i++) {
                            g_CurrentROMIconGfx[i].raw_val = gfx::ConvertColor<gfx::abgr8888::Color, gfx::abgr1555::Color>(icon_gfx[i]).raw_val;
                        }
                        delete[] icon_gfx;
                        ASSERT_TRUE(g_CurrentROMIcon.CreateLoadFrom(false, 0, 40, g_CurrentROMIconGfx, gfx::IconWidth, gfx::IconHeight, false));
                        g_CurrentROMIcon.CenterXInScreen();

                        const auto sys_lang = util::GetSystemLanguage();
                        const auto title_text = util::ConvertFromUnicode(rom.banner.game_titles[static_cast<u32>(sys_lang)]);
                        ASSERT_TRUE(GetTextFont().RenderText(title_text, 0, 40 + 32 + 8, NormalLineHeight, ScreenWidth, false, gfx::abgr8888::BlackColor, true, g_CurrentROMText));
        ...
        void OnOtherFatInput(const u32 k_down) {
            if(k_down & KEY_B) {
                CleanTextIcon(true);
                if(g_CurrentDirectory.empty()) {
                    ...
                    const auto new_path = fs::GetAbsolutePath(g_CurrentDirectory + "/..");
                    LoadFatFileBrowseMenu(new_path);
                }
            }
        }
            ...
            if(k_down & KEY_B) {
                ...
                if(g_NitroFsEntryStack.empty()) {
                    ...
                    if((cur_entry.file_ref_ptr != entry.file_ref_ptr) && !entry.is_loaded_narc) {
                        ...
                        g_CurrentDirectory = fs::RemoveLastEntry(g_CurrentDirectory);
                        g_CurrentDirectory = fs::RemoveLastEntry(g_CurrentDirectory);
        ...
        void LoadTextureImpl(const bool is_fat, const std::string &ncgr_name, const std::string &nclr_name, const std::string &nscr_name) {
            ...
            const auto expected_single_plt_size = gfx::GetPaletteColorCountForPixelFormat(nclr.palette.pix_fmt) * sizeof(gfx::xbgr1555::Color);
            ...
            if(!gfx::ConvertGraphicsToRgba(ctx)) {
                ShowOkDialog("Unable to load texture...");
                return;
            }
            auto gfx_buf = util::NewArray<gfx::abgr1555::Color>(ctx.out_width * ctx.out_height);
            for(u32 i = 0; i < ctx.out_width * ctx.out_height; i++) {
                gfx_buf[i].raw_val = gfx::ConvertColor<gfx::abgr8888::Color, gfx::abgr1555::Color>(ctx.out_rgba[i]).raw_val;
            }
            const auto plt_color_count = nclr.palette.data_size / sizeof(gfx::xbgr1555::Color);
            auto plt_copy = util::NewArray<gfx::abgr1555::Color>(plt_color_count);
            const auto plt = reinterpret_cast<gfx::xbgr1555::Color*>(nclr.data);
            ...
            if(ShowGraphicEditor(gfx_buf, plt_copy, plt_color_count, ctx.out_width, ctx.out_height)) {
                auto ok = false;
                gfx::RgbaToGraphicsContext ctx_2 = {
                ...
                RunWithDialog("Converting texture...", [&]() {
                    for(u32 i = 0; i < ctx.out_width * ctx.out_height; i++) {
                        ctx.out_rgba[i].raw_val = gfx::ConvertColor<gfx::abgr1555::Color, gfx::abgr8888::Color>(gfx_buf[i]).raw_val;
                    }
                    ok = gfx::ConvertRgbaToGraphics(ctx_2);
                });
                if(!ok) {
                ...
                ncgr.data = util::NewArray<u8>(ctx_2.out_gfx_data_size);
                ...
                nclr.data = util::NewArray<u8>(ctx_2.out_plt_data_size);
                ...
                if(has_nscr) {
                    ...
                    nscr.data = util::NewArray<u8>(ctx_2.out_scr_data_size);
        ...
        void OnSelect(const bool is_fat, const std::vector<std::string> &selected_items) {
            if(selected_items.size() >= 2) {
                ...
                for(const auto &sel_item : selected_items) {
                    const auto sel_ext = util::ToLowerString(fs::GetFileExtension(sel_item));
                    if(sel_ext == "ncgr") {
                        if(!ncgr.empty()) {
                            ok = false;
                            break;
    ...
    void InitializeFileBrowseMenu() {
        u32 w;
        u32 h;
        ASSERT_TRUE(LoadPNGGraphics("ui/dir_icon.png", ntr::nfs::CreateSelfNitroFsFileHandle(), w, h, g_DirectoryIconGfx));
        ASSERT_TRUE(w == ScrollMenuIconWidth);
        ASSERT_TRUE(h == ScrollMenuIconHeight);

        ASSERT_TRUE(LoadPNGGraphics("ui/file_icon.png", ntr::nfs::CreateSelfNitroFsFileHandle(), w, h, g_FileIconGfx));
        ASSERT_TRUE(w == ScrollMenuIconWidth);
        ASSERT_TRUE(h == ScrollMenuIconHeight);

        ASSERT_TRUE(LoadPNGGraphics("ui/rom_icon.png", ntr::nfs::CreateSelfNitroFsFileHandle(), w, h, g_ROMIconGfx));
        ASSERT_TRUE(w == ScrollMenuIconWidth);
        ASSERT_TRUE(h == ScrollMenuIconHeight);

        ASSERT_TRUE(LoadPNGGraphics("ui/music_icon.png", ntr::nfs::CreateSelfNitroFsFileHandle(), w, h, g_MusicIconGfx));
        ASSERT_TRUE(w == ScrollMenuIconWidth);
        ASSERT_TRUE(h == ScrollMenuIconHeight);

        ASSERT_TRUE(LoadPNGGraphics("ui/tex_icon.png", ntr::nfs::CreateSelfNitroFsFileHandle(), w, h, g_TexIconGfx));
        ASSERT_TRUE(w == ScrollMenuIconWidth);
        ASSERT_TRUE(h == ScrollMenuIconHeight);
    }
    ...
    void LoadFatFileBrowseMenu(const std::string &base_dir) {
		...
		FS_FOR_EACH_FAT_ENTRY(base_dir, {
            ...
            if(is_dir) {
                entries.push_back({
                    ...
                    .text = entry_name,
                    .on_select = std::bind(OnFatDirectorySelect, entry_path),
        ...
        LoadScrollMenu(entries, OnOtherFatInput);
    ...
    void LoadNitroFsFileBrowseMenu(ntr::nfs::NitroFsFileFormat *file_ref_ptr, ntr::nfs::NitroDirectory *dir_ref_ptr, const bool is_narc, const std::string &base_path) {
        ...
        std::vector<ScrollMenuEntry> entries;
        if(!file_ref_ptr->DoWithReadFile([&](fs::BinaryFile &bf) {
            if(base_path.empty()) {
    ...
    void RestoreFileBrowseMenu() {
        if(g_NitroFsEntryStack.empty()) {
            LoadFatFileBrowseMenu(g_CurrentDirectory);
        }
        else {
            RestoreNitroFsFileBrowseMenuImpl();
        }
    ...
    bool SaveFile(const std::string &file_path, const std::string &fmt, std::function<bool(const std::string&, bool)> on_save) {
        ...
        if(prev_is_fat) {
            ...
            const auto file_name = fs::GetFileName(file_path);
            ...
            path_copy = fs::GetBaseDirectory(file_path) + "/" + file_name_copy;
        }
        auto ok = false;
