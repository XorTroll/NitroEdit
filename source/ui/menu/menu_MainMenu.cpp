...
namespace ui::menu {
    ...
    void InitializeMainMenu() {
        ASSERT_SUCCESSFUL(g_Logo.CreateLoadFrom(false, 0, 0, "ui/logo.png", CreateSelfNitroFsFileHandle()));
        g_Logo.CenterInRegion(6, 6, 244, 161);
        
        u32 w;
        u32 h;
        ASSERT_SUCCESSFUL(LoadPNGGraphics("ui/fs_icon.png", CreateSelfNitroFsFileHandle(), w, h, g_FsIconGfx));
        ASSERT_TRUE(w == ScrollMenuIconWidth);
        ASSERT_TRUE(h == ScrollMenuIconHeight);

        ASSERT_SUCCESSFUL(LoadPNGGraphics("ui/about_icon.png", CreateSelfNitroFsFileHandle(), w, h, g_AboutIconGfx));
        ASSERT_TRUE(w == ScrollMenuIconWidth);
        ASSERT_TRUE(h == ScrollMenuIconHeight);
    ...
    void LoadMainMenu() {
        ASSERT_SUCCESSFUL(g_Logo.EnsureLoaded());
